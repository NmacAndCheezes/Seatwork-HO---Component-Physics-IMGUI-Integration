...
MeshColor::MeshColor(const wchar_t* full_path) : AMesh(full_path)
{
	...
	std::vector<VertexColor> list_vertices;
	...
	for (size_t s = 0; s < shapes.size(); s++)
	{
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
		{
			...
			for (unsigned char v = 0; v < num_face_verts; v++)
			{
				...
				VertexColor vertex = VertexColor(Vector3D(vx, vy, vz), Vector3D(1, 1, 1));
	...
	m_vbColor = new VertexColorBuffer(
		&list_vertices[0], 
		sizeof(VertexMesh),
...
const VertexBufferPtr& MeshColor::getVertexBuffer()
{
	return (VertexBufferPtr)m_vbColor;
}
